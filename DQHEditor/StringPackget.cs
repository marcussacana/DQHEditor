...
namespace DQHEditor {
    ...
    public class StringPackget {

        struct Entry{
            internal int Offset;
            internal int Length;
        }
        public static File[] Open(byte[] Packget) {
            StructReader Reader = new StructReader(new System.IO.MemoryStream(Packget));
            ...
            for (int i = 0; i < Count; i++) {
                ...
                Files[i] = new File() {
                    Index = i,
                    Content = GetSubArray(Packget, Entry.Offset, Entry.Length)
                };
            }
            return Files;
        }
